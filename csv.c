...
int writeCourseInfo(struct course new_course) // 杈撳叆璇剧▼淇℃伅骞跺皢鍏跺啓鍏ユ枃浠禼ourse.csv
{
    FILE *file;
    file = fopen("course.csv", "a");
    if (file == NULL) {
...
struct course* readCourseInfo(struct course course_info[]) // 鎺ュ彈涓�涓暟缁勶紝灏嗘枃浠朵腑鐨勮绋嬩俊鎭鍏ユ暟缁勪腑,骞惰繑鍥炴寚閽?
{
    FILE *file;
    int i = 0;
    ...
    course_info[i].course_property[0] = '\0'; // 瀛楃涓叉竻绌?
    course_info[i].course_name[0] = '\0';     // 瀛楃涓叉竻绌?
...
struct student* readStudentInfo(struct student student_info[]) // 璇诲彇鏂囦欢student.csv涓殑瀛︾敓淇℃伅骞惰繑鍥炰竴涓暟缁?
{
    FILE *file;
    ...
    char line[MAX_VALUE];
    int i = 0; // 灏? i 绉诲埌杩欓噷锛岄伩鍏嶆瘡娆″惊鐜兘閲嶇疆
    while (1) {
        if (fgets(line, sizeof(line), file) == NULL) {
            break;
        }
        ...
        token = strtok(line, ",");
        int student_id = atoi(token);
        sprintf(student_info[i].student_id, "%d", student_id); // 灏嗘暣鏁拌浆鎹负瀛楃涓?
        ...
        student_info[i].student_selectedCourseId[50] = -1; // 璇剧▼閫夋嫨鏁扮粍鐨勭粨鏉熸爣璁?
...
int writeStudentInfo(struct student new_student) // 杈撳叆瀛︾敓淇℃伅骞跺皢鍏跺啓鍏ユ枃浠秙tudent.csv
{
    FILE *file;
